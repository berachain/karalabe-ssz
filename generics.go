...
type commonBytesLengths interface {
	// fork | nonce | address | verkle-stem | hash | pubkey
	~[4]byte | ~[8]byte | ~[20]byte | ~[31]byte | ~[32]byte | ~[48]byte |
		// committee | signature | bloom | blob
		~[64]byte | ~[96]byte | ~[256]byte | ~[131072]byte
}
